...
class SwiGLU(nn.Module):
    def __init__(self, d_model, d_ff, device=None, dtype=None):
        super().__init__()
        self.up_gate = nn.Linear(d_model, 2 * d_ff, bias=False, device=device, dtype=dtype)  # fused
        self.down    = nn.Linear(d_ff, d_model, bias=False, device=device, dtype=dtype)

    def forward(self, x):
        u, g = self.up_gate(x).chunk(2, dim=-1)  # [*, d_ff] each
...
class RotaryPositionalEmbedding(nn.Module):
    ...
    def forward(self, x, token_positions):
        # Assume positions are just [0..L-1] for all batches/heads
        # x: [B, H, L, d]
        B, H, L, d = x.shape
        sin = self.sin[:L].to(dtype=x.dtype, device=x.device)  # [L, d/2]
        cos = self.cos[:L].to(dtype=x.dtype, device=x.device)
...
class MultiHeadSelfAttention(nn.Module):
    def __init__(self, d_model, num_heads, max_seq_len=None, theta=None, device=None, dtype=None):
        ...
        self.qkv = nn.Linear(d_model, 3 * num_heads * self.d, bias=False, device=device, dtype=dtype)
        self.proj = nn.Linear(num_heads * self.d, d_model, bias=False, device=device, dtype=dtype)
        self.rope = RotaryPositionalEmbedding(theta, self.d, max_seq_len, device=device) if theta else None

    def forward(self, x, token_positions=None):
        B, L, _ = x.shape
        qkv = self.qkv(x)  # [B, L, 3*h*d]
        qkv = qkv.view(B, L, self.h, 3 * self.d).transpose(1, 2)  # [B, h, L, 3d]
        q, k, v = torch.split(qkv, self.d, dim=-1)               # each [B, h, L, d]

        if self.rope:
            if token_positions is None:
                token_positions = None  # (we'll use simple slice broadcast in RoPE)
            q = self.rope(q, token_positions)  # broadcasted, no advanced indexing
            k = self.rope(k, token_positions)

        y = F.scaled_dot_product_attention(q, k, v, is_causal=True)  # [B, h, L, d]
        y = y.transpose(1, 2).contiguous().view(B, L, self.h * self.d)  # [B, L, h*d]
        return self.proj(y)
